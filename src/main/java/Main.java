...
import views.ProfileEditPageView;
...
public class Main 
{
	...
	{		
		...
		//profile page
		ProfilePageview profilePageView = new ProfilePageview(databaseInterface, fileStoreInterface);
		
		//Profile edit page
		ProfileEditPageView editProfile = new ProfileEditPageView(databaseInterface, fileStoreInterface);
		...
		while(shouldKeepRunning)
		{
			...
			for (int i = 0; i < requestToProcess.size(); i++) 
			{
	        	...
	        	if(toProcess.path.length()==0) {
	        		toProcess.path = "index";
	        		indexViewPage.process(toProcess);
	        	}
	        	else if(indexViewPage.process(toProcess))
		        {
		        	//index page is processed
		        }
		        else if(profilePageView.process(toProcess)) {
		        	//profile page is processed
		        }
		        else if(editProfile.process(toProcess)) {
		        	//edit profile page is processed
		        	toProcess.path = "profilePage";
		        	profilePageView.process(toProcess);
		        }
		        else
		        {
